...
@pytest.mark.skip(reason="测试用例需要根据实际实现重写 - 待实现")
class TestRunSTQuantization:
    """测试 ST 官方量化脚本调用"""

    @pytest.mark.unit
    def test_quantization_fake_mode(self, adapter, temp_saved_model, tmp_path):
        """测试 fake 量化模式（无校准数据集）"""
        ...
        with patch("subprocess.run") as mock_run:
            ...
            with patch.object(adapter, "_prepare_quant_config") as mock_config:
                mock_config.return_value = tmp_path / "config.yaml"
    ...
    @pytest.mark.unit
    def test_quantization_with_calibration(self, adapter, temp_saved_model, temp_calibration_zip, tmp_path):
        """测试使用真实校准数据集的量化"""
        output_dir = tmp_path / "quantized"
        output_dir.mkdir()
    ...
    @pytest.mark.unit
    def test_quantization_different_input_sizes(self, adapter, temp_saved_model, tmp_path):
        """测试不同输入尺寸的量化配置"""
        output_dir = tmp_path / "quantized"
        output_dir.mkdir()
        ...
        with patch("subprocess.run") as mock_run:
            ...
            with patch.object(adapter, "_prepare_quant_config") as mock_config:
                ...
                for size in input_sizes:
                    ...
                    mock_config.assert_called_once()
                    call_args = mock_config.call_args
                    assert call_args[0][1] == size  # input_size 参数

    @pytest.mark.unit
    def test_quantization_script_failure(self, adapter, temp_saved_model, tmp_path):
        """测试量化脚本执行失败"""
        output_dir = tmp_path / "quantized"
        output_dir.mkdir()
    ...
    @pytest.mark.unit
    def test_quantization_output_file_not_created(self, adapter, temp_saved_model, tmp_path):
        """测试量化文件未生成"""
        output_dir = tmp_path / "quantized"
        output_dir.mkdir()
...
class TestValidateQuantizedModel:
    ...
    def test_validate_model_success(self, adapter, temp_tflite_file):
        """测试成功验证量化模型"""
        with patch("tensorflow.lite.Interpreter") as mock_interpreter_cls:
            # Mock interpreter
        ...
        with patch.object(adapter, "_export_to_saved_model") as mock_export:
            with patch.object(adapter, "_run_st_quantization") as mock_quant:
                with patch.object(adapter, "_validate_quantized_model") as mock_validate:
                    with patch.object(adapter, "_prepare_ne301_project") as mock_prepare:
                        with patch.object(adapter, "_build_ne301_model") as mock_build:

                            # Mock 各步骤返回值
                            mock_export.return_value = "/tmp/saved_model"
                            mock_quant.return_value = "/tmp/quantized.tflite"
                            mock_validate.return_value = True
                            mock_prepare.return_value = Path("/tmp/ne301/Model")
                            mock_build.return_value = "/tmp/output.bin"
                ...
                with patch.object(adapter, "_export_to_saved_model", return_value="/tmp/saved_model"):
                    with patch.object(adapter, "_run_st_quantization", return_value="/tmp/quantized.tflite"):
                        with patch.object(adapter, "_validate_quantized_model", return_value=True):
                            with patch.object(adapter, "_prepare_ne301_project", return_value=Path("/tmp/ne301")):
                                with patch.object(adapter, "_build_ne301_model", return_value="/tmp/output.bin"):
                                    result = adapter.convert_model(
                                        task_id=task_id,
                                        model_path=temp_model_file,
                                        config=sample_config
...
class TestPerformance:
        ...
        with patch("subprocess.run") as mock_run:
            ...
            with patch.object(adapter, "_prepare_quant_config") as mock_config:
                ...
                adapter._run_st_quantization(
                    ...
                    input_size=640,
                    calib_dataset_path=str(large_zip),
    ...
    @pytest.mark.skip(reason="测试用例需要根据实际实现重写 - 待实现")
    def test_memory_cleanup_during_quantization(self, adapter):
        """测试量化过程中的内存清理"""
        # 这个测试验证量化脚本正确处理了内存
        # 通过检查配置中的 max_calib_images 参数
        with patch.object(adapter, "_prepare_quant_config") as mock_config:
            mock_config.return_value = "/tmp/config.yaml"
