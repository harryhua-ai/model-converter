...
def main(cfg: DictConfig) -> None:
    ...
    def representative_data_gen():
        ...
        if cfg.quantization.fake is True:
            ...
            if len(list_of_files) > max_calib_images:
                print(f"⚠️  校准图片数量 ({len(list_of_files)}) 超过限制，"
                      f"随机采样 {max_calib_images} 张")
                random.seed(42)
            ...
            for image_file in tqdm.tqdm(list_of_files):
                try:
                    ...
                    print(f"⚠️  Warning: Error processing {image_file}: {e}, skipping")
                    continue
