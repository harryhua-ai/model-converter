...
class DockerToolChainAdapter:
    ...
    def convert_model(
        ...
        try:
            ...
            task_manager.add_log(
                task_id, f"输入尺寸: {config['input_size']}x{config['input_size']}"
            )
    ...
    def _export_to_quantized_tflite(
        ...
        try:
            ...
            tm.add_log(
                task_id, f"✅ TFLite 验证通过: input=uint8, output=int8, shape={list(output_shape)}"
            )
    ...
    def _prepare_ne301_project(
        ...
        if tflite_input_size > 0 and tflite_input_size != config_input_size:
            ...
            logger.error(
                f"  这会导致 bin 文件过大（预期 ~{config_input_size*3//256} MB → 实际 ~{tflite_input_size*3//256} MB）"
            )
        ...
        logger.info(
            f"✅ 配置参数: input_size={config['input_size']}, num_classes={config['num_classes']}"
        )
    ...
    def _make_model(
        ...
        try:
            ...
            if target_output and target_output.exists():
                ...
                logger.info(
                    f"  ✓ make 流程完成，输出文件: {target_output.name} ({file_size:,} bytes)"
                )
                tm.add_log(
                    task_id, f"✅ NE301 打包完成: {target_output.name} ({file_size // 1024}KB)"
                )
                ...
                if result["StatusCode"] != 0:
                    ...
                    raise RuntimeError(
                        f"make pkg-model 失败 (exit code {result['StatusCode']})，未生成输出文件。"
                    )
                else:
                    logger.error(f"  ✗ make 成功但未在 build/ 目录找到任何 *_pkg.bin 文件")
                    logger.error(
                        f"  build/ 目录内容: {list(build_dir.iterdir()) if build_dir.exists() else 'dir not found'}"
                    )
                    raise RuntimeError(
    ...
    def _run_make_in_container(self, ne301_project_path: Path) -> None:
        ...
        try:
            ...
            if exit_code != 0:
                ...
                raise RuntimeError(
                    f"make model 失败: exit code {exit_code}。"
                    f"解决建议: 请检查 NE301 容器状态，确保容器内依赖完整。"
                    f"可以查看详细日志了解具体错误原因。"
                )
    ...
    def _add_ota_header(
        ...
        if process.returncode != 0:
            raise RuntimeError(
                f"OTA 打包失败: {' '.join(output[-10:])}。"
                f"解决建议: 请检查 NE301 SDK 版本，确保 OTA 打包工具可用且配置正确。"
            )
    ...
    def _validate_quantized_model(
        ...
        try:
            ...
            if input_size in expected_boxes:
                ...
                if actual_boxes != expected:
                    error_msg = f"输出形状错误: 预期 (1, 34, {expected})，" f"实际 {output_shape}"
